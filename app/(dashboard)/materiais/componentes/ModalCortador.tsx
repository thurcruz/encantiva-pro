...
interface Props {
  titulo: string
  imagemUrl: string        // URL pública da imagem no storage
  onClose: () => void
}
...
export default function ModalCortador({ titulo, imagemUrl, onClose }: Props) {
  ...
  // Carrega a imagem a partir da URL do material
  useEffect(() => {
    setCarregando(true)
    setErro(null)
    ...
    img.onerror = () => { setErro('Não foi possível carregar a imagem. Tente baixar o original.'); setCarregando(false) }
    img.src = imagemUrl
  }, [imagemUrl])
  ...
  async function gerarPDF() {
    ...
    try {
      ...
      a.href = url
      a.download = `painel_${titulo.replace(/\s/g, '_')}.pdf`
      a.click()
          ...
          {!carregando && !erro && imagem && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div>
                  <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '13px', color: '#111827', margin: '0 0 2px' }}>Orientação das folhas</p>
