  ...
  async function salvar() {
    ...
    try {
      if (cliente) {
        ...
        const { data, error } = await supabase
          .from('clientes')
          .insert({ ...form, nome: form.nome.trim(), usuario_id: usuarioId })
          .select().single()
        if (error) { setErro('Erro ao salvar: ' + error.message); return }
        onSalvo(data as Cliente)
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      <div>
        ...
        <input style={inputStyle} placeholder="Nome do cliente" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} />
      </div>
      ...
      <div>
        ...
        <input style={inputStyle} placeholder="Rua, número, bairro..." value={form.endereco} onChange={e => setForm(f => ({ ...f, endereco: e.target.value }))} />
      </div>
      <div>
      ...
      <div>
        ...
        <textarea rows={3} style={{ ...inputStyle, resize: 'vertical' }} placeholder="Preferências, informações extras..." value={form.observacoes} onChange={e => setForm(f => ({ ...f, observacoes: e.target.value }))} />
      </div>
...
export default function ClienteDetalhes({ cliente: clienteInicial, pedidos, contratos, usuarioId }: Props) {
  ...
  const totalPedidos = pedidos.reduce((acc, p) => acc + Number(p.valor_total), 0)
  const totalContratos = contratos.reduce((acc, c) => acc + Number(c.valor_total), 0)
  const totalGeral = totalPedidos + totalContratos
  ...
  return (
    <div>
      ...
      {!editando && (
        <div style={cardStyle}>
          <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '11px', color: '#9ca3af', margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '0.5px' } as React.CSSProperties}>Dados pessoais</p>
      ...
      <div style={cardStyle}>
        ...
        {abaAtiva === 'pedidos' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {pedidos.length === 0 ? (
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', color: '#d1d5db', textAlign: 'center', padding: '24px 0', margin: 0 }}>Nenhum pedido registrado</p>
            ) : pedidos.map(p => {
        ...
        {abaAtiva === 'contratos' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {contratos.length === 0 ? (
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', color: '#d1d5db', textAlign: 'center', padding: '24px 0', margin: 0 }}>Nenhum contrato registrado</p>
            ) : contratos.map(c => {
