...
export default async function PaginaPlanosPage() {
  ...
  let cpfCnpjSalvo: string | null = null

  if (user) {
    const [{ data: assinatura }, { data: perfil }] = await Promise.all([
      supabase.from('assinaturas').select('plano, status, trial_expira_em').eq('usuario_id', user.id).single(),
      supabase.from('perfis').select('cpf_cnpj').eq('id', user.id).single(),
    ])
    ...
    if (assinatura) {
      ...
      if (assinaturaAtiva || trialAtivo) planoAtual = assinatura.plano ?? null
      statusAtual = assinatura.status ?? null
    }

    const cpf = perfil?.cpf_cnpj ?? ''
cpfCnpjSalvo = (cpf.length === 11 || cpf.length === 14) ? cpf : null
  }
